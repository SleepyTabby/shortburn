...
public class PlayerInputHandler : MonoBehaviour
{
    ...
    private void Update()
    {
        keyCoolDown = Mathf.Clamp(keyCoolDown -= Time.deltaTime, 0, 10);
    }

    public bool CanProcessInput()
    {
        return Cursor.lockState == CursorLockMode.Locked && !m_GameFlowManager.GameIsEnding && m_GameFlowManager.allowPlayerToMove;
    }

    public float keyCoolDown;
    public bool pressedKey(KeyCode key, float coolDown = .5f)
    {
        if (keyCoolDown == 0 && Input.GetKeyDown(key))
        {
